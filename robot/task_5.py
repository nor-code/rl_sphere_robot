...
class TrakingTrajectoryTask5(base.Task):
    ...
    def get_termination(self, physics):
        if len(self.points) == 0 or physics.data.time > self.timeout or self.count_invalid_states >= 7 \
               or len(self.points) == self.achievedPoints:  #or self.count_hard_invalid_state >= 6 :
    ...
    def get_reward(self, physics):
        ...
        missed_index2 = self.index1 + 1
        missed_index3 = self.index2 + 2
        missed_index4 = self.index3 + 3

        reward = distance_reward1 + distance_reward2 + distance_reward3 + distance_reward4
        - 50 * abs(self.index2 - missed_index2) - 50 * abs(self.index3 - missed_index3) - 50 * abs(self.index4 - missed_index4)
    ...
    # check TODO если робот повернул назад, обычно бывает из-за резкого поворота назад
    def is_invalid_state_hard(self):
        x, y = self.robot_position
        if self.prev_index1 == self.index1:
            new_dist = np.linalg.norm([x - self.points[self.index1][0], y - self.points[self.index1][1]])
            ...
            if self.index1 >= 0 and len(self.points) - 1 == self.prev_index1:
                return False
            if self.index1 < self.prev_index1:
                return True
            if self.index1 > self.prev_index1:
                self.dist = np.linalg.norm([x - self.points[self.index1][0], y - self.points[self.index1][1]])
                return False
