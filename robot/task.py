...
class TrakingTrajectoryTask(base.Task):
    ...
    def initialize_episode(self, physics):
        physics.named.data.qpos[0:3] = [0, 0, 0.19]
    ...
    def get_termination(self, physics):
        ...
        if len(self.points) == 0 or physics.data.time > self.timeout or self.__distance_to_current_point(x, y) >= 0.5:
            return 0.0
    ...
    def get_reward(self, physics):
        ...
        cos_a = np.dot(PC, PO) / (np.linalg.norm(PC) * np.linalg.norm(PO))
